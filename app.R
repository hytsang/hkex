...
server <- function(input, output) {
  ...
  output$tablenet <- renderDataTable({
      ...
      noticestablenet %>% datatable(escape = FALSE, colnames = c("Listed company", "Stock code", "Shareholder/director name", "Change in position (%)")) %>% formatRound(columns = "sharesdiff", digits = 2)
  })
}
